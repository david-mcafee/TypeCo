...
/***/ function(module, exports, __webpack_require__) {
	...
	var Game = function () {
	  ...
	  _createClass(Game, [{
	    ...
	    key: "createExplosion",
	    value: function createExplosion(position, time) {
	      // if (Date.now() < time + 3000) {
	      this.explosions.push(new Explosion(position, time));
	    }
	  }, {
	    key: "draw",
	    value: function draw(context) {
	      ...
	      if (this.explosions.length > 0) {
	        for (var _i = 0; _i < this.explosions.length; _i++) {
	          if (Date.now < this.explosions[_i].time + 3000) {
	            this.explosions[_i].draw();
	          }
	        }
	      }
	    }
