...
class Game {
  ...
  addMissile(options) {
    this.missiles.push(
      new Missile({
        ...
        game: this,
        color: "red",
        word: Game.dictionaryWords.words[
          Math.floor(Math.random() * Game.dictionaryWords.words.length)
        ],
        radius: 0,
        speed: this.speed,
      })
  ...
  drawGamePlay(context) {
    ...
    context.fillRect(
      0,
      Game.Y_DIMENSION * (17 / 20),
      Game.X_DIMENSION,
      Game.Y_DIMENSION - Game.Y_DIMENSION * (17 / 20)
    );

    // COLLISION WITH BOTTOM OF SCREEN
    for (let i = 0; i < this.missiles.length; i++) {
      if (
        this.missiles[i].position[1] + this.missiles[i].radius >=
        Game.Y_DIMENSION * (17 / 20)
      ) {
        ...
        for (var idx2 = 0; idx2 < this.cities.length; idx2++) {
          ...
          if (
            this.missiles[i].position[0] >= this.cities[idx2].position[0] &&
            this.missiles[i].position[0] <=
              this.cities[idx2].position[0] + this.cities[idx2].width
          ) {
    ...
    if (this.selectedWord.length > 0) {
      if (this.selectedWord[0].position[1] >= Game.Y_DIMENSION * (17 / 20)) {
        ...
        for (var idx2 = 0; idx2 < this.cities.length; idx2++) {
          // COLLISION WITH A CITY OCCURED
          if (
            this.selectedWord[0].position[0] >= this.cities[idx2].position[0] &&
            this.selectedWord[0].position[0] <=
              this.cities[idx2].position[0] + this.cities[idx2].width
          ) {
            this.cities[idx2].decreaseHealth(pointsDeducted * 20);
  ...
  drawLevelComplete(context) {
    ...
    context.fillText(
      ...
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (7 / 10)
    );
    context.fillText(
      "esc: start over",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (8 / 10)
    );
  }
  ...
  drawStartScreen(context) {
    ...
    context.fillText(
      "ENTER: PLAY / PAUSE",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (24 / 80)
    );
    context.fillStyle = "blue";
    context.fillText(
      "ESC: END GAME",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (27 / 80)
    );
    context.fillStyle = "GREEN";
    context.fillText(
      "DELETE/BACKSPACE: UNSELECT",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (30 / 80)
    );
    ...
    context.fillText(
      "Instructions",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (36 / 80)
    );
    context.fillText(
      "Word “missiles” are falling from the sky!",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (39 / 80)
    );
    context.fillText(
      "Type them before they hit the cities.",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (42 / 80)
    );
    //
    context.fillText(
      "When you start typing a word, it will be selected",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (45 / 80)
    );
    context.fillText(
      "Once you select a word,",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (48 / 80)
    );
    context.fillText(
      "any typos will cost you points!",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (51 / 80)
    );
    //
    ...
    context.fillText(
      "TIP:",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (57 / 80)
    );
    context.fillText(
      "Didn’t select the word you wanted?",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (60 / 80)
    );
    context.fillText(
      "Type delete/backspace.",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (63 / 80)
    );
    //
    ...
    context.fillStyle = "green";
    context.fillText(
      "NOTE: Since the words are missiles (i.e. `enemies`), PLEASE",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (69 / 80)
    );
    context.fillText(
      "contact me if you find a word that shouldn't be there!",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (72 / 80)
    );
  }
  ...
  drawPauseScreen(context) {
    ...
    context.fillText(
      "enter: continue playing",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (12 / 20)
    );
    context.fillText(
      "esc: start over",
      Game.X_DIMENSION / 2,
      Game.Y_DIMENSION * (13 / 20)
    );
  }
    ...
    if (keyEntered === "enter") {
      if (this.startScreen === true) {
        ...
        if (this.gameOver === true) {
          ...
          this.pause = false;
          this.timeEnteredNewLevel =
            this.timeEnteredNewLevel + (Date.now() - this.timePaused);
        }
      }
      ...
      if (this.selectedWord.length === 0) {
        for (let i = 0; i < this.missiles.length; i++) {
          if (
            ...
            this.missiles[i].word.split("")[0] === keyEntered
          ) {
        ...
        if (
          this.selectedWord.length !== 0 &&
          this.selectedWord[0].word.split("").length === 0
        ) {
          // CREATE EXPLOSION
