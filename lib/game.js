...
class Game {
  ...
  drawGamePlay(context) {
    ...
    for (let i = 0; i < this.missiles.length; i++) {
      if ( (this.missiles[i].position[1] + this.missiles[i].radius) >= Game.Y_DIMENSION * (17/20)) {
        ...
        let pointsDeducted = this.missiles[i].word.split("").length * 200;
        this.totalPoints -= pointsDeducted;
        ...
        for (var idx2 = 0; idx2 < this.cities.length; idx2++) {
          ...
          if (this.missiles[i].position[0] >= this.cities[idx2].position[0] &&
            this.missiles[i].position[0] <= (this.cities[idx2].position[0] + this.cities[idx2].width) ) {
              this.cities[idx2].decreaseHealth(pointsDeducted / 200);

              if (this.cities[idx2].health > 360) {
    ...
    if (this.selectedWord.length > 0) {
      ...
      if (this.selectedWord[0].position[1] >= (Game.Y_DIMENSION * (17/20) ) ) {
        ...
        let pointsDeducted = this.selectedWord[0].word.split("").length * 200;
        this.totalPoints -= pointsDeducted;
  ...
  drawGameOver(context){
    ...
    context.textBaseline = "middle";
    context.fillStyle = "white";
    context.fillText("enter: start over", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (2/3) );
  }
  ...
  handleMissiles(context){
    ...
    console.log(this.missileCreationRate);
  ...
  drawStartScreen(context) {
    ...
    context.fillText("TYPE COMMAND", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (2/10));
      ...
      // }
      //
      // var maxWidth = Game.X_DIMENSION * (23/25);
      // var lineHeight = 35;
      // var x = (Game.X_DIMENSION * (1/25));
      // var y = 60;
      // console.log(x);
    ...
    context.textAlign = "center";
    context.fillStyle = "yellow";
    context.fillText("Instructions", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (35/80));
    context.fillText("Word “missiles” are falling from the sky - your", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (38/80));
    context.fillText("job is to type them before they hit the ground", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (41/80));
    //
    context.fillText("When you start typing a word, it will be SELECTED.", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (44/80));
    context.fillText("You must finish typing this word before it hits", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (47/80));
    //

    context.fillText("the ground. CAREFUL! Once you select a word,", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (50/80));
    context.fillText("any typos will cost you points!", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (53/80));
    //

    context.fillStyle = "red";
    context.fillText("TIPS:", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (58/80));
    context.fillText("Didn’t select the word you wanted? Hit delete or", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (61/80));
    context.fillText("backspace to deselect it and type the word you", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (64/80));
    //
    context.fillText("want. Once you unselect a word, it will continue to", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (67/80));
    context.fillText("fall at it’s original speed (faster than when selected)", Game.X_DIMENSION / 2, Game.Y_DIMENSION * (70/80));
